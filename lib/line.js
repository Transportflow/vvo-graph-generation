 ...
 * @property {[[string]]} routes defined trips, order of stops that real vehicles drive
 *
...
const getTrip = (line, stop) => {
  return new Promise(async (resolve, reject) => {
    ...
    response.data.Stops.forEach((stop, index) => {
      ...
      edges.push(new JgfEdge(response.data.Stops[index - 1].Id, stop.Id, "regional", undefined, {lines: [line.name], tripIds: [line.id], time: timeDiff}))
    })
    resolve(edges)
  })
}
 ...
 * @returns {[string]} all networks along with their operators
 */
const listNetworks = (lines) => {
  const networks = {}
  ...
  Object.keys(lines).forEach((lineId) => {
    ...
    let network = networks[line.id.substring(0, 3)]
    if (!network)
      network = {
        // TODO: add line count for network and operator
        operators: [line.operator]
      }
    else if (!network.operators.includes(line.operator)) network.operators.push(line.operator)

    networks[line.id.substring(0, 3)] = network
  })
